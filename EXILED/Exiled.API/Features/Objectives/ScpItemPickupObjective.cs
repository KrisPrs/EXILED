...
namespace Exiled.API.Features.Objectives
{
    ...
    public class ScpItemPickupObjective : HumanObjective<PickupObjectiveFootprint>, IWrapper<BaseObjective>
    {
        ...
        public void AddItem(Player target, Item item, Pickup pickup) => this.Base.OnItemAdded(target.ReferenceHub, item.Base, pickup.Base);
        ...
        /// <param name="target">Target player.</param>
        /// <param name="item">Item that was picked up.</param>
        public void AddItem(Player target, Item item) => this.Base.OnItemAdded(target.ReferenceHub, item.Base, item.CreatePickup(Vector3.zero, Quaternion.identity, false).Base);
        ...
        /// <param name="target">Target player.</param>
        /// <param name="pickup">Pickup that was picked up.</param>
        public void AddItem(Player target, Pickup pickup) => this.Base.OnItemAdded(target.ReferenceHub, Item.Create(pickup.Type, target).Base, pickup.Base);
        ...
        public void AddItem(Player target, ItemType type) => this.AddItem(target, Item.Create(type, target));
    }
}