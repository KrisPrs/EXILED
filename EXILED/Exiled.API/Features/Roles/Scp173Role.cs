...
namespace Exiled.API.Features.Roles
{
    ...
    public class Scp173Role : FpcRole, ISubroutinedScpRole, IHumeShieldRole, ISpawnableScp
    {
            ...
            : base(baseRole)
        {
            ...
            this.MovementModule = this.FirstPersonController.FpcModule as Scp173MovementModule;

            if (!this.SubroutineModule.TryGetSubroutine(out Scp173ObserversTracker scp173ObserversTracker))
                Log.Error("Scp173ObserversTracker not found in Scp173Role::ctor");

            this.ObserversTracker = scp173ObserversTracker;

            if (!this.SubroutineModule.TryGetSubroutine(out Scp173BlinkTimer scp173BlinkTimer))
                Log.Error("Scp173BlinkTimer not found in Scp173Role::ctor");

            this.BlinkTimer = scp173BlinkTimer;

            if (!this.SubroutineModule.TryGetSubroutine(out Scp173TeleportAbility scp173TeleportAbility))
                Log.Error("Scp173TeleportAbility not found in Scp173Role::ctor");

            this.TeleportAbility = scp173TeleportAbility;

            if (!this.SubroutineModule.TryGetSubroutine(out Scp173TantrumAbility scp173TantrumAbility))
                Log.Error("Scp173TantrumAbility not found in Scp173Role::ctor");

            this.TantrumAbility = scp173TantrumAbility;

            if (!this.SubroutineModule.TryGetSubroutine(out Scp173AudioPlayer scp173AudioPlayer))
                Log.Error("Scp173AudioPlayer not found in Scp173Role::ctor");

            this.AudioPlayer = scp173AudioPlayer;

            if (!this.SubroutineModule.TryGetSubroutine(out Scp173BreakneckSpeedsAbility scp173BreakneckSpeedsAbility))
                Log.Error("Scp173BreakneckSpeedsAbility not found in Scp173Role::ctor");

            this.BreakneckSpeedsAbility = scp173BreakneckSpeedsAbility;
        }
        ...
        {
            get => this.BreakneckSpeedsAbility.Cooldown.Remaining;
            set
            {
                this.BreakneckSpeedsAbility.Cooldown.Remaining = value;
                this.BreakneckSpeedsAbility.ServerSendRpc(true);
            }
        }
        ...
        {
            get => this.TantrumAbility.Cooldown.Remaining;
            set
            {
                this.TantrumAbility.Cooldown.Remaining = value;
                this.TantrumAbility.ServerSendRpc(true);
            }
        }
        ...
        /// </summary>
        public bool IsObserved => this.ObserversTracker.IsObserved;

        /// <summary>
        /// Gets a <see cref="IEnumerable{T}"/> of players that are currently viewing SCP-173. Can be empty.
        /// </summary>
        public IEnumerable<Player> ObservingPlayers => this.ObserversTracker.Observers.Select(x => Player.Get(x));

        /// <summary>
        /// Gets SCP-173's max move speed.
        /// </summary>
        public float MaxMovementSpeed => this.MovementModule.MaxMovementSpeed;

        /// <summary>
        /// Gets the SCP-173's movement speed.
        /// </summary>
        public override float MovementSpeed => this.MovementModule.ServerSpeed;
        ...
        {
            get => this.ObserversTracker.SimulatedStare;
            set => this.ObserversTracker.SimulatedStare = value;
        }
        ...
        {
            get => this.BlinkTimer.AbilityReady;
            set
            {
                if (value)
                {
                    this.BlinkTimer._endSustainTime = -1;
                    this.BlinkTimer._totalCooldown = 0;
                    this.BlinkTimer._initialStopTime = NetworkTime.time;
                }
                else
                {
                    this.BlinkTimer.ResetObject();
                    this.BlinkTimer._observers.UpdateObservers();
                }

                this.BlinkTimer.ServerSendRpc(true);
            }
        }
        ...
        {
            get => this.BlinkTimer.RemainingBlinkCooldown;
            set
            {
                this.BlinkTimer._initialStopTime = NetworkTime.time;
                this.BlinkTimer._totalCooldown = value;
                this.BlinkTimer.ServerSendRpc(true);
            }
        }
        ...
        /// </summary>
        public float BlinkDistance => this.TeleportAbility.EffectiveBlinkDistance;
        ...
        {
            get => this.BreakneckSpeedsAbility.IsActive;
            set => this.BreakneckSpeedsAbility.IsActive = value;
        }
        ...
        {
            if (failIfObserved && this.IsObserved)
                return null;

            this.TantrumAbility.Cooldown.Trigger(cooldown);

            return this.Owner.PlaceTantrum();
        }
        ...
        public void SendAudio(Scp173AudioPlayer.Scp173SoundId soundId) => this.AudioPlayer.ServerSendSound(soundId);

        /// <summary>
        /// Teleport SCP-173 using the blink ability to the Target Position.
        /// </summary>
        /// <param name="targetPos">The Target Position.</param>
        public void Blink(Vector3 targetPos) => this.BlinkTimer.ServerBlink(targetPos);
